...
public class RegisterAsNewUser implements ClientManager{
	
	Client c;
	
	public static void main(String[] args) throws UnknownHostException, IOException{
		Socket s = new Socket("localhost", 8338);
		new RegisterAsNewUser(s);
	}
	
	public RegisterAsNewUser(Socket s) throws IOException{
		...
		byte[] email = "admin@supercool.com".getBytes();
		
		byte[][] info = {username, pass, email};
		this.c.sendPacket(new DynamicNumberOptionsPacket(info, 0));
	}
